...
def hypothesis(theta, X):
    return 1 / (1 + np.exp(-(np.dot(theta.T, X.T)))) - 0.0000001
...
def log_reg2(file):
    ...
    theta1 = gradient_descent(X1, Y1, theta1, 0.02, 500)
    print(theta1)
    ...
    for i in range(0, 3):
        ...
        print(thetaVal1)
        print("BEFORE H")
        h = hypothesis(np.matrix(thetaVal1[0]), X1)
        print(h)
        print("AFTER H")
        output.append(h)
        print(output)
    accuracy = 0
    output_numpy_array = np.asarray(output)
    for col in range(0, 3):
        for z in range(0, 3):
            for row in range(len(Y1)):
                if output_numpy_array[col][z][row].min() >= 0.7:
                    accuracy += 1
    accuracy = accuracy / output_numpy_array.shape[0]
